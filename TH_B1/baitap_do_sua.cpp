...
int pourWaterFullX (State cur_state, State *result){
    if (cur_state.X == Xmax || cur_state.B == empty){
        return 0;
    }
    result->X = min(cur_state.B + cur_state.X, Xmax);
    result->B = max(cur_state.B - (Xmax - cur_state.X), empty);
    result->Y = cur_state.Y;
    return 1;
}

int pourWaterFullY (State cur_state, State *result){
    if (cur_state.Y == Xmax || cur_state.B == empty){
        return 0;
    }
    result->Y = min(cur_state.B + cur_state.Y, Ymax);
    result->B = max(cur_state.B - (Ymax - cur_state.Y), empty);
...
int pourWaterEmptyX (State cur_state, State *result){
    ...
    result->B = cur_state.B + cur_state.X;
    result->X = empty;   
    result->Y = cur_state.Y;
    return 1;
}
...
int pourWaterEmptyY (State cur_state, State *result){
    ...
    result->B = cur_state.B + cur_state.Y;
    result->Y = empty;
    result->X = cur_state.X;
    return 1;
}
...
void print_state (State cur_state){
    printf ("\nX:%d ---- Y:%d ---- B:%d", cur_state.X, cur_state.Y, cur_state.B);
}
